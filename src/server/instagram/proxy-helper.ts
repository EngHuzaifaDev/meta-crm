...
function getOrCreateProxyAgent(): ProxyAgent | null {
  ...
  const normalized = /^[a-zA-Z][a-zA-Z0-9+.-]*:\/\//.test(proxyUrl)
    ? proxyUrl
    : `http://${proxyUrl}`
...
async function buildProxyAuthExtension(config: ProxyConfig): Promise<string | null> {
  ...
  zip.file("background.js", `chrome.webRequest.onAuthRequired.addListener(
  (details, callback) => {
    callback({
      authCredentials: {
...
export async function verifyProxyIP(): Promise<{
  ...
  try {
    const { statusCode, body } = await undiciRequest("https://httpbin.org/ip", {
      dispatcher: agent,
      method: "GET",
    ...
    if (statusCode !== 200) {
      return { ip: "(error)", ok: false, error: `httpbin returned ${statusCode}` }
    }

    const text = await body.text()
    const data = JSON.parse(text)
    return { ip: data.origin || "unknown", ok: true }
  } catch (err: any) {
